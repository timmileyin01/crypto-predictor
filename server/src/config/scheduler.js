...
async function runDailyJob() {
  ...
  try {
    ...
    console.error(
      "[Scheduler] ⚠️ Price fetch failed, continuing with existing data:",
      err.message,
    );
  }
  ...
  for (const { symbol } of symbols) {
    try {
      ...
      console.log(
        `[Scheduler] ✅ ${symbol} → $${mlResult.predicted_close} for ${predictedFor}`,
      );

      const { checkAndFireAlerts } =
        await import("../controllers/alertsController.js");
      ...
      try {
        ...
        for (const alert of alerts) {
          ...
          if (triggered) {
            ...
            if (
              alert.triggered_at &&
              new Date(alert.triggered_at) > oneHourAgo
            ) {
              continue;
            }
            ...
            console.log(
              `[Alerts] Live alert fired for ${alert.email} — ${symbol} ${alert.condition} $${alert.threshold}`,
            );
          } else {
            await pool.query(
        ...
        // Wait 2 seconds between each symbol to respect rate limits
...
export function startScheduler() {
  ...
  // Change from */5 to */15
  cron.schedule("*/15 * * * *", checkLivePriceAlerts);
  console.log("[Scheduler] Live price alerts checking every 15 minutes");
}
